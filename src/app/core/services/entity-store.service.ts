...
export class EntityStoreService {
  ...
  resolve$(db: string, id: any, property: string = 'name'): Observable<any> {
    ...
    if (!db) {
      return of(normalizedId);
    }
    if (db === 'tables' && (normalizedId === 'Hızlı Satış' || normalizedId === 'Paket Servis')) {
      return of(normalizedId);
    }
    ...
    const stream = defer(() => from(this.mainService.getData(db, normalizedId))).pipe(
      map((doc: any) => {
        if (!doc) {
          return normalizedId;
        }
  ...
  resolveTableName(tableId: any): Promise<string> {
    return this.resolveEntity('tables', tableId, 'name');
  }
