...
export class CashboxComponent implements OnInit {
    ...
    if (!form._id) {
      ...
      this.mainService.addData('cashbox', schema).then(res => {
        this.logService.createLog(
          logType.CASHBOX_CREATED,
          res.id,
          `Kasaya ${(form.cash + form.card + form.coupon)} tutarında ${this.type} eklendi.`
        );
        this.fillData();
  ...
  removeData(id) {
    this.mainService.removeData('cashbox', id).then(res => {
      this.logService.createLog(
        logType.CASHBOX_DELETED,
        res.id,
        `Kasadan '${this.selectedData.description}' adlı ${this.type}'i silindi.`
      );
  ...
  fillData() {
    ...
    this.mainService.getAllBy('cashbox', {}).then(result => {
      ...
      try {
        this.incomes = this.cashboxData
          .filter(obj => obj.type === 'Gelir')
          .map(obj => obj.card + obj.cash + obj.coupon)
          .reduce((a, b) => a + b);
      } catch (error) {
        this.incomes = 0;
      }
      try {
        this.outcomes = this.cashboxData
          .filter(obj => obj.type === 'Gider')
          .map(obj => obj.card + obj.cash + obj.coupon)
          .reduce((a, b) => a + b);
      } catch (error) {
        this.outcomes = 0;
      ...
      this.mainService.getAllBy('reports', { type: 'Store' }).then(res => {
        let report = res.docs;
        report = report.filter(obj => obj.connection_id && obj.connection_id !== 'Genel')
