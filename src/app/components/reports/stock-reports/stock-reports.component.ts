...
export class StockReportsComponent implements OnInit {
  ...
  fillData() {
    ...
    this.mainService.getAllBy('stocks', {}).then(result => {
      ...
      this.allStocks = this.allStocks.sort((b, a) =>
        (b.left_total / (b.total * b.first_quantity)) * 100 - (a.left_total / (a.total * a.first_quantity)) * 100);
      this.stocksView = this.allStocks.sort((b, a) =>
        (b.left_total / (b.total * b.first_quantity)) * 100 - (a.left_total / (a.total * a.first_quantity)) * 100);
    });
    ...
    this.mainService.getAllBy('logs', {}).then(res => {
      this.stockLogs = res.docs.filter(obj =>
        obj.type >= logType.STOCK_CREATED && obj.type <= logType.STOCK_CHECKPOINT).sort((a, b) => b.timestamp - a.timestamp);
    });
  }
}
